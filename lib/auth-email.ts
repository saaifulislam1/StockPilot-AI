  ...
  return `
    <div style="font-family: Arial, sans-serif; padding: 32px; background: #f8fafc; color: #0f172a;">
      <div style="max-width: 560px; margin: 0 auto; background: white; border-radius: 20px; padding: 32px; border: 1px solid #e2e8f0;">
        <p style="margin: 0 0 12px; font-size: 12px; letter-spacing: 0.2em; color: #64748b; text-transform: uppercase;">StockPilot Auth</p>
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  await resend.emails.send({
    ...
    to: email,
    subject: "Verify your StockPilot account",
    html: emailLayout(
      "Verify your email",
